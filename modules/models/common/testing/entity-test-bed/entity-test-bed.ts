 ...
 * @private
 */
function assertEntity<T extends BaseEntity>(entity: T, assertion: EntityAssertion<T>, parentKey: string = ''): void {
...
async function assertDatabase(previous: DatabaseSnapshot, databaseAssertion: DatabaseAssertion): Promise<void> {
  for (const data of Object.entries<TableAssertion>(databaseAssertion)) {
    ...
    if (tableAssertion.created) {
      const newlyCreatedEntities = currentEntities.filter((record) => gtStrictly(record.createdAt, latestUpdatedTime));
